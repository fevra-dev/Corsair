...
@click.option('-o', '--output', type=click.Choice(['console', 'json', 'html', 'sarif']), 
...
def scan(
    ...
    if min_score > 0 and report.average_score < min_score:
        click.echo(
            click.style(
                f"\nFailed: Average score {report.average_score:.1f} < {min_score}",
                fg="red"
            ),
            err=True
        )
    ...
    try:
        ...
        if as_json:
            ...
            for record in records:
                ...
                click.echo(
                    f"{date}  Score: "
                    + click.style(f"{score}/100 ({grade})", fg=color)
                )
