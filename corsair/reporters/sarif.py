...
class SARIFReporter(BaseReporter):
    ...
    def generate(self, report: ScanReport) -> str:
        ...
        logger.info(
            f"[SARIF] Generated {len(results)} results with {len(self._rules)} rules"
        )
        ...
        for target in report.results:
            for finding in target.findings:
                ...
                result = {
                    ...
                    "level": self.SEVERITY_TO_LEVEL[finding.severity],
                    "message": {
                        "text": self._format_message(finding, target.url)
                    },
                    "locations": [{
                        "physicalLocation": {
                            "artifactLocation": {
        ...
        return [{
            ...
            "taxa": [
                ...
                },
                {
                    ...
                    "name": "Injection",
                    "shortDescription": {
                        "text": "Injection vulnerabilities including XSS"
                    }
                }
            ]
        }]
    ...
    def _get_tags(self, finding: Finding) -> List[str]:
        ...
        tags = [
            "security",
            f"header/{finding.header.lower().replace('-', '_')}"
        ]
