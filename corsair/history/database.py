...
class HistoryDatabase:
    ...
    def _get_previous_scan(
        self, cursor: sqlite3.Cursor, url: str, current_id: int
    ) -> Optional[Dict]:
        ...
        row = cursor.fetchone()
        if row:
            return {"id": row[0], "score": row[1], "grade": row[2], "findings": json.loads(row[3])}
        return None
    ...
    def _detect_drift(
        ...
        current_issues = {
            f["title"]
            for f in [f.to_dict() for f in current.findings]
            if f["severity"] in ["CRITICAL", "HIGH"]
        }
        previous_issues = {
            f["title"] for f in previous["findings"] if f["severity"] in ["CRITICAL", "HIGH"]
        }
    ...
    def compare_scans(self, scan_id_1: int, scan_id_2: int) -> Optional[HistoricalComparison]:
        """
        Compare two scans.
    ...
    def get_drift_alerts(self, url: Optional[str] = None, limit: int = 50) -> List[Dict]:
        """
        Get drift alerts.
